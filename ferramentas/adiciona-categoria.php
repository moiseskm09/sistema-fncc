...
if (isset($_POST['nomeCategoriaN'])) {
    ...
    if($insereCategoria == 1){
     $buscaCodCategoria = mysqli_query($conexao,"SELECT cod_categoria FROM categoria_documentos WHERE categoria = '$nomeCategoriaN' LIMIT 1");
    $resultado = mysqli_fetch_assoc($buscaCodCategoria);
    $codCategoria = $resultado["cod_categoria"];    
    mkdir("../arquivos/modelos_documentos/categoria_$codCategoria", 0777, true);
    header("location: ../sistema/incluir-doc.php?sucesso=3");
    }else{
