...
if(isset($_GET['cooperativaExtF'], $_GET["periodo"])){
    $cooperativaExtF = $_GET['cooperativaExtF'];
    $periodo = $_GET['periodo'];
    $sql_buscaCDD = mysqli_query($conexao, "SELECT * FROM canal_de_denuncias INNER JOIN cooperativas ON  cdd_coop = cod_coop WHERE cdd_periodo like '%$periodo%' and cdd_coop like '%$cooperativaExtF%'");
//    $sql_buscaObs = mysqli_query($conexao, "SELECT * FROM canal_de_denuncias INNER JOIN cooperativas ON  ext_coop = cod_coop WHERE cdd_periodo like '%$periodo%' and cdd_coop like '%$cooperativaExtF%'");
    ...
    $sql = mysqli_query($conexao, "SELECT cod_denuncia FROM canal_de_denuncias");
    ...
    $sql_buscaCDD = mysqli_query($conexao, "SELECT * FROM canal_de_denuncias INNER JOIN cooperativas ON  cdd_coop = cod_coop LIMIT $inicio, $itens_por_pagina ");
...
<html lang="pt-br">
    <head>
        ...
        <title>Inclusão de Relátorios de Denúncias</title>
...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
  ...
  <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-1 pb-1 mb-1 border-bottom">
                      ...
                      <span class="breadcrumb-item active text-success">Inclusão de Relatórios</span>
                  </div>
                            <div class="btn-toolbar mb-2 mb-md-0">
                                <div class="mr-2">
                                    <a class="btn btn-sm btn-warning mb-1" onClick="history.go(-1)"><i class="uil uil-angle-left"></i> Voltar</a>
                                    <a class="btn btn-sm btn-success mb-1" href="#addDocumento" data-toggle="modal" data-target="#addDocumento"><i class="uil uil-plus"></i> Adicionar Relatório</a>
                                    <?php if($filtroON === 1){ ?>
                                    <a class="btn btn-sm btn-dark mb-1" href="incluir-rel-denuncias.php"><i class="uil uil-filter-slash"></i> Limpar Filtro</a>
                                    <?php } ?>
                                    ...
                                    if ($numeroLinhas > 0) {
                                        while ($resultadoCDD = mysqli_fetch_assoc($sql_buscaCDD)) {
                                            ...
                                            <tr class="linha-hover">
                                                ...
                                                <td class="text-center" style="font-size:15px;"><?php echo $resultadoCDD['cooperativa']; ?></td>
                                                ...
                                                <a title="Deletar Relatório" href="../ferramentas/deleta-denuncia.php?cod_coop=<?php echo $resultadoCDD['cdd_coop']; ?>&nome_cdd=<?php echo $resultadoCDD['cdd_arquivo']; ?>&cod_denuncia=<?php echo $resultadoCDD['cod_denuncia']; ?>" desativar-confirm="Tem certeza de que deseja o item selecionado?"><i class="bi bi-trash text-white btn-sm btn-danger"></i></a>
                                                </td> 
                                            </tr>
                                            <?php                                          
                                        }
...
<div class="modal fade" id="addDocumento" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body card-fundo-body">
          <form action="../ferramentas/inclui-rel-denuncias.php" method="POST" enctype="multipart/form-data">
            <div class="row">
       <div class="col-lg-12 col-md-12 col-12">
    <div class="form-floating mb-3">
      <select class="form-select pesquisa-select" id="cooperativaCDD" name="cooperativaCDD" required>
          <option value="">Selecione</option>
        <?php
        $buscaCooperativas = mysqli_query($conexao, "SELECT cod_coop, cooperativa FROM cooperativas");
        while ($resultadoCooperativas = mysqli_fetch_assoc($buscaCooperativas)) {
        ...
        <label for="cooperativaExtCN">Cooperativa <span class="text-danger">*</span></label>
                ...
                <div class="col-lg-12 col-md-12 col-12">
                          <div class="form-floating mb-3">
                            <input type="text" name="periodo" id="periodo" class="form-control" placeholder="Insira o período" autocomplete="off" required>
                            <label for="jurosCapitalExtCN">Período <span class="text-danger">*</span></label>
                          </div>  
                        </div>
              
              <div class="col-lg-12 col-md-12 col-12">
                <div class="form-group files">
                  <input type="file" class="form-control" name="arquivoCDD" id="arquivoCDD" required>
                </div>
              </div>
            </div>
      </div>
...
<!-- fim modal documento -->
            ...
            <div id="liveToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="4000">
                ...
                <div class="toast-body" style="background-color: #f5c2c7; color: #1c1d3c;"">
                    <span>Não foi possível incluir o relatório! Tente Novamente!</span>
                </div>
            </div>
        </div>'; 
                }
