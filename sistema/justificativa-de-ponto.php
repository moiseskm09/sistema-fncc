...
if(isset($_GET["pessoa"], $_GET["dataRefIncialF"], $_GET["dataRefFinalF"])){
$primeiroDia = $_GET["dataRefIncialF"];
$UltimodiaDia = $_GET["dataRefFinalF"];
$pessoa = $_GET["pessoa"];
$buscaJustificativa = mysqli_query($conexao, "SELECT * FROM justificativa_ponto INNER JOIN controle_de_ponto ON just_user = ponto_user and just_dia = ponto_dia WHERE just_user = '$pessoa' and just_dia >= '$primeiroDia' and just_dia <= '$UltimodiaDia'");
$filtro = 1;
}else{
$filtro = 0;
$pessoa = "";
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-2 pb-2 mb-2 border-bottom">
                            <div class="breadcrumb mb-2 mb-md-0" style="--bs-breadcrumb-divider: '>'; font-size: 16px;">
                                ...
                                <span class="breadcrumb-item active text-success">Justificativa de Ponto</span>
                            </div>
         ...
         echo '<option selected value='.$resultadoPessoa['id_usuario'].'>'.$resultadoPessoa['nome'].'</option>';                                                                   
                                                  ...
                                                  <div class="col-lg-4 col-md-4 col-12">
                                                        ...
                                                        <div class="col-12 text-end">
                                                            <button type="submit" id="filtrarAtendimento" class="btn btn-sm btn-success"><i class="bi bi-funnel"></i> Buscar Justificativas</button>
                                                        </div>
                                ...
                                <?php if($filtro == 1){ ?>
                                            ...
                                            <table class="table table-borderless" style= "white-space: nowrap;">
                                                ...
                                                <tbody class="p-0">
                                                    ...
                                                    <?php
                                                    if (mysqli_num_rows($buscaJustificativa) > 0) {
                                                        while ($resultadoJustificativa = mysqli_fetch_assoc($buscaJustificativa)) {
                                                            ?>
                                                            <tr class="linha-hover info-td">
                                                                <td class="info-td fw-bold cor-primaria"><?php echo utf8_encode(strftime('%a - %d/%m', strtotime($resultadoJustificativa["just_dia"]))); ?></td>
                                                                <td class="info-td cor-primaria">
                                                    <?php 
                                                    if($resultadoJustificativa["ponto_hora_extra"] != null && $resultadoJustificativa["ponto_hora_extra"] != "00:00:00"){
                                                        echo "Hora Extra";
                                                    }elseif($resultadoJustificativa["ponto_hora_atraso"] != null && $resultadoJustificativa["ponto_hora_atraso"] != "00:00:00" && $resultadoJustificativa["ponto_situacao"] == "1" ){
                                                        echo "Atraso";  
                                                    }elseif($resultadoJustificativa["ponto_situacao"] == "2"){
                                                        echo "Ausência";
                                                                ...
                                                                <td class="info-td cor-primaria"><?php echo $resultadoJustificativa["just_motivo"];?></td>
                                                                <td class="info-td text-success"><?php 
                                                if($resultadoJustificativa['just_situacao'] == 0){
                                                echo '<span class="badge badge-warning text-dark rounded-pill d-inline">PENDENTE</span>';
                                                }elseif($resultadoJustificativa['just_situacao'] == 1){
                                                    echo '<span class="badge badge-danger rounded-pill d-inline">REPROVADA</span>';
                                                }elseif($resultadoJustificativa['just_situacao'] == 2){
                                                    echo '<span class="badge badge-success rounded-pill d-inline">APROVADA</span>';
                                                }
                                                ?></td>
                                                                <td class="info-td text-success fw-bold">
                                                                    <?php if($resultadoJustificativa["just_arquivo"] != null){ ?>
                                                                        <a href="../ferramentas/download-justificativa.php?nome_arquivo=<?php echo $resultadoJustificativa['just_arquivo']; ?>" title="Dowload do Anexo"><i class="bi bi-download btn-sm btn-info"></i></a>
                                                                    <?php } ?>
                                                                    
                                                                    <a href="../ferramentas/aprova-justificativa.php?cod_just=<?php echo $resultadoJustificativa["cod_just"]; ?>&justificativa=aprovada&cod_ponto=<?php echo $resultadoJustificativa["cod_ponto"]; ?>&cod_user=<?php echo $resultadoJustificativa["just_user"]; ?>&data_inicial=<?php echo $primeiroDia; ?>&data_final=<?php echo $UltimodiaDia; ?>" title="Aprovar Justificativa"><i class="bi bi-check2-square btn-sm btn-success"></i></a>
                                                    <a href="../ferramentas/aprova-justificativa.php?cod_just=<?php echo $resultadoJustificativa["cod_just"]; ?>&justificativa=reprovada&cod_ponto=<?php echo $resultadoJustificativa["cod_ponto"];?>&cod_user=<?php echo $resultadoJustificativa["just_user"]; ?>&data_inicial=<?php echo $primeiroDia; ?>&data_final=<?php echo $UltimodiaDia; ?>" title="Reprovar Justificativa" desativar-confirm="Tem certeza de que deseja excluir o item selecionado?"><i class="bi bi-x-lg btn-sm btn-danger"></i></a></td>
                                                            </tr>
                                                            <?php
                                                        }
                                                    } else {
...
<?php
            ...
            <div id="liveToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="4000">
                ...
                <div class="toast-body" style="background-color: #a3cfbb; color: #1c1d3c;"">
                    <span>Justificativa Atualizada!</span>
                </div>
            </div>
        </div>';
            }
           ...
           echo '<div class="toast-container position-fixed bottom-0 end-0 p-3">
            <div id="liveToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="4000">
                ...
                <div class="toast-body" style="background-color: #f5c2c7; color: #1c1d3c;"">
                    <span>Erro ao atualizar informações! Tente novamente!</span>
                </div>
            </div>
        </div>'; 
        }
