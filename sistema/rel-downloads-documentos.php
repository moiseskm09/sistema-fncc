...
if (isset($_GET['dataRefIncialF'], $_GET["dataRefFinalF"])) {
    ...
    $dataRefFinalF = $_GET['dataRefFinalF'];
        ...
        $sql_buscaDownloads = mysqli_query($conexao, "SELECT * FROM download_termo_aceito INNER JOIN usuarios ON dta_user = id_usuario INNER JOIN cooperativas ON user_coop = cod_coop WHERE dta_data >= '$dataRefIncialF' and dta_data <= '$dataRefFinalF' ORDER BY dta_data DESC LIMIT 100");
    $filtroON = 1;
} else {
        $sql_buscaDownloads = mysqli_query($conexao, "SELECT * FROM download_termo_aceito INNER JOIN usuarios ON dta_user = id_usuario INNER JOIN cooperativas ON user_coop = cod_coop ORDER BY dta_data DESC LIMIT 100");
    $filtroON = 0;
}
?>
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid">
                      ...
                      <span class="breadcrumb-item active text-success">Download de Documentos</span>
                  </div>
                            <div class="btn-toolbar mb-2 mb-md-0">
                                <div class="mr-2">
                                    <a class="btn btn-sm btn-warning" onClick="history.go(-1)"><i class="uil uil-angle-left"></i> Voltar</a>
<?php if ($filtroON === 1) { ?>
                                        <a class="btn btn-sm btn-dark" href="rel-downloads-documentos.php"><i class="uil uil-filter-slash"></i> Limpar Filtro</a>
                                    <?php } ?>
...
<?php
                                    if ($sql_buscaDownloads > 0) {
                                        while ($resultadoDownloads = mysqli_fetch_assoc($sql_buscaDownloads)) {
                                            ?>
                                            <tr class="linha-hover">
                                                <td style="font-size:15px;"><?php echo ucfirst($resultadoDownloads['nome']); ?></td>
                                                <td style="font-size:15px;"><?php echo $resultadoDownloads['cooperativa']; ?></td>
                                                <td style="font-size:15px;"> 
                                                <?php echo $resultadoDownloads['dta_arquivo']; ?>
                                                </td>
                                                <td style="font-size:15px;"><?php echo strftime('%d/%m/%Y', strtotime($resultadoDownloads['dta_data'])); ?></td>
