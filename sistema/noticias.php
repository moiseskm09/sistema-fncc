...
if (isset($_GET['nomeNoticia'])) {
    $nomeNoticia = $_GET['nomeNoticia'];
    $sql_buscaNoticia = mysqli_query($conexao, "SELECT * FROM site_noticias where titulo_noticia LIKE '%$nomeNoticia%'");
    $numeroLinhas = mysqli_num_rows($sql_buscaNoticia);
    $filtroON = 1;
} else {
    ...
    $sql = mysqli_query($conexao, "SELECT cod_noticia FROM site_noticias LIMIT 100");
    ...
    $sql_buscaNoticia = mysqli_query($conexao, "SELECT cod_noticia, titulo_noticia, categoria_noticia, publicado, data_noticia FROM site_noticias ORDER BY publicado ASC, data_noticia DESC LIMIT $inicio, $itens_por_pagina ");
    $numeroLinhas = mysqli_num_rows($sql_buscaNoticia);
    $filtroON = 0;
}
?>
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-1 pb-1 mb-1 border-bottom">
                            <div class="breadcrumb mb-2 mb-md-0" style="--bs-breadcrumb-divider: '>'; font-size: 16px;">
                                ...
                                <span class="breadcrumb-item active text-success">Notícias</span>
                            </div>
                            <div class="btn-toolbar mb-2 mb-md-0">
                                <div class="mr-2">
                                    <a class="btn btn-sm btn-warning mb-1" onClick="history.go(-1)"><i class="uil uil-angle-left"></i>Voltar</a>
                                    <a class="btn btn-sm btn-success mb-1" href="incluir-noticia.php"><i class="uil uil-plus"></i> Adicionar</a>
                                    <?php if ($filtroON === 1) { ?>
                                        <a class="btn btn-sm btn-dark mb-1" href="noticias.php"><i class="uil uil-filter-slash"></i> Limpar Filtro</a>
                                    <?php } ?>
                        ...
                        <div class="table-responsive">
                            <table class="table table-borderless table-sm bg-white">
                                ...
                                <tbody class="bg-white p-0">
                                    <?php
                                    if ($numeroLinhas > 0) {
                                        while ($resultadoNoticias = mysqli_fetch_assoc($sql_buscaNoticia)) {
                                            ?>
                                            <tr class="linha-hover">
                                                <td><?php echo substr($resultadoNoticias['titulo_noticia'], 0, 65); ?></td>
                                                <td><?php echo $resultadoNoticias['categoria_noticia']; ?></td>
                                                <td><?php echo strftime('%d/%m/%y', strtotime($resultadoNoticias['data_noticia'])); ?></td>
                                                <td><?php
                                                    if ($resultadoNoticias['publicado'] == 1) {
                                                        echo '<span class="badge badge-success rounded-pill d-inline">Publicado</span>';
                                                    } else {
                                                        echo '<span class="badge badge-warning rounded-pill d-inline">Salvo</span>';
                                                    }
                                                    ?></td>
                                                <td class="text-center">
                                                    <a href="editar-noticia.php?id=<?php echo $resultadoNoticias['cod_noticia']; ?>"><i class="bi bi-pencil-square text-dark btn-sm btn-warning"></i></a>
                                                    <a href="../ferramentas/desativa-noticia.php?id=<?php echo $resultadoNoticias['cod_noticia']; ?>" desativar-confirm="Tem certeza de que deseja excluir o item selecionado?"><i class="bi bi-trash text-white btn-sm btn-danger"></i></a>
                                                </td> 
                                            </tr>
                                            <?php
                                        }
...
?>
                                            ...
                                            <div class="row">
                                                <div class="col-lg-12 col-md-12 col-12">
                                                    <div class="form-floating mb-3">
                                                        <input type="text" name="nomeNoticia" class="form-control focoInput" id="nomeNoticia" placeholder="Título Notícia" autocomplete="off" required>
                                                        <label for="nomeNoticia">Título da Notícia</label>
                                                    </div>  
                                                </div>                   
                                            </div>
                                    </div>
            ...
            <div id="liveToast" class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="4000">
                ...
                <div class="toast-body" style="background-color: #f5c2c7; color: #1c1d3c;"">
                    <span>Erro ao deletar notícia! Tente Novamente!</span>
                </div>
            </div>
        </div>';
                }
